...
if ( isset( $_POST[ 'submit' ] ) ) {
    ...
    if ( $oldpass != $newpass ) {
        $sql = "SELECT * FROM penjual WHERE idpenjual = '$idpenjual' AND password = '$oldpass'";
        $result = mysqli_query( $sambungan, $sql );
        $num_rows = mysqli_num_rows( $result );
        if ( $num_rows > 0 ) {
            $sqlupdatepass = "UPDATE `penjual` SET `password`= '$newpass' WHERE idpenjual = '$idpenjual'";
            $resultupdate = mysqli_query($sambungan , $sqlupdatepass);
            if ($resultupdate){
                echo "<script>alert('Operasi Berjaya');window.location='/kedai/penjual/tukar.php';</script>";
            }
        }else{
            echo "<script>alert('Operasi Gagal');window.location='/kedai/penjual/tukar.php';</script>";
        }
    }else{
        echo "<script>alert('Operasi Gagal');window.location='/kedai/penjual/tukar.php';</script>";
    }
